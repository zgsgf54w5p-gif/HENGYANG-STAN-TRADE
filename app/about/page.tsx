...
export default function AboutPage() {
  return (
    <main className="bg-[radial-gradient(circle_at_top_left,_#f8fbff_0%,_#f2f6fb_45%,_#eef3f9_100%)] text-slate-800">
      <section className="relative overflow-hidden bg-[linear-gradient(135deg,_#0F172A_0%,_#2563EB_55%,_#3B82F6_100%)] text-white">
        ...
        <div className="relative mx-auto flex max-w-6xl flex-col items-center px-6 py-20 text-center sm:px-8 lg:px-12 lg:py-28">
          <div className="max-w-3xl">
            <p className="text-sm font-bold uppercase tracking-[0.3em] text-orange-400">About Hengyang Stan Trade</p>
            <p className="font-script mt-7 text-4xl text-orange-300 sm:text-5xl">Trade with intention</p>
      ...
      <section className="mx-auto max-w-7xl px-6 py-20 sm:px-8 lg:px-12 lg:py-28">
        <div className="mx-auto max-w-4xl text-center">
          <p className="text-sm font-bold uppercase tracking-[0.3em] text-[#2563EB]">Who we are</p>
          <h2 className="mt-4 text-4xl font-black leading-tight text-[#0F172A] sm:text-5xl">Built around clarity, consistency and long-term trade relationships.</h2>
        </div>
      ...
      <section className="border-y border-slate-200 bg-white">
        <div className="mx-auto max-w-7xl px-6 py-20 sm:px-8 lg:px-12 lg:py-24">
          <div className="mx-auto max-w-3xl text-center">
            <p className="text-sm font-bold uppercase tracking-[0.3em] text-[#2563EB]">What we bring</p>
            <h2 className="mt-4 text-3xl font-black text-[#0F172A] sm:text-4xl">Practical support for every stage of sourcing.</h2>
          </div>
          <div className="mt-12 grid gap-6 md:grid-cols-3">
            {capabilities.map((item) => (
              <article key={item.number} className="rounded-[1.5rem] border border-slate-200 bg-[#F8FAFC] p-7 text-center shadow-sm transition hover:-translate-y-1 hover:shadow-lg">
                ...
                <h3 className="mt-6 text-xl font-bold text-[#0F172A]">{item.title}</h3>
      ...
      <section className="mx-auto max-w-7xl px-6 py-20 sm:px-8 lg:px-12 lg:py-28">
        <div className="mx-auto max-w-3xl text-center">
          <p className="text-sm font-bold uppercase tracking-[0.3em] text-[#2563EB]">How we work</p>
          <h2 className="mt-4 text-3xl font-black leading-tight text-[#0F172A] sm:text-4xl">A straightforward path from idea to shipment.</h2>
        ...
        <div className="mt-12 grid gap-x-8 gap-y-10 sm:grid-cols-2">
          {process.map(([number, title, text]) => (
            <div key={number} className="rounded-[1.5rem] border border-slate-200 bg-white p-6 text-center shadow-sm">
              ...
              <h3 className="mt-3 text-xl font-bold text-[#0F172A]">{title}</h3>
